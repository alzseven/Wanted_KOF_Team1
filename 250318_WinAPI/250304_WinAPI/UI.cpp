...
void UI::Update()
{
    ...
    if (character1)
        character1LeftHp = (character1->GetHealth() / 100.0f) * (HpBarSize - HpBarOffset);
    if (character2)
        character2LeftHp = (character2->GetHealth() / 100.0f) * (HpBarSize - HpBarOffset);
...
void UI::Render(HDC hdc)
{
    ...
    if(character1HpValid)
        character1HpValid->RenderRange(hdc, 71, 43, 71 + character1LeftHp, 9);
    if (character1HpValid)
